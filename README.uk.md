...
Застосунок перевіряє не лише MIME-тип, а й **магічні байти** кожного завантажуваного PDF-файлу. Перші 4 байти кожного завантажуваного файлу звіряються із сигнатурою PDF (`%PDF`, hex `25 50 44 46`). Це унеможливлює ситуацію, коли файл лише перейменовано в `.pdf`, але фактично він має інший формат.

Зображення перевіряються лише за MIME-типом та розширенням файлу — перевірка магічних байтів не застосовується.
...
src/
  ...
  hooks/
    ...
    useFileUpload.ts
    useUrlSync.ts
    useStorageInit.ts  # перевірка доступності сховища при старті
    useTabSync.ts      # синхронізація метаданих між вкладками
    useDebounce.ts
    useMediaQuery.ts
  ...
  lib/
    ...
    tabSyncHelpers.ts  # хелпери BroadcastChannel
